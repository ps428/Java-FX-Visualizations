...
public class BTree<K extends Comparable<K>> implements Serializable {
    ...
    public BTNode<K> nullBTNode = new BTNode<K>();
     ...
     LinkedList<BTree<K>> stTrees = new LinkedList<BTree<K>>();//making a linked list of keys
    ...
    BTNode<K> getHalfKeys(K key, BTNode<K> fullNode) {
     ...
     void mergeWithParentNode(BTNode<K> childNode, int index) {
     ...
     void mergeWithParentNode(BTNode<K> childNode) {
        int ParentNodeSize = childNode.getParent().getSize();
        ...
        if (ParentNodeSize == childNode.getParent().getSize()) {
            mergeWithParentNode(childNode, ParentNodeSize);
        }
        for (int i = 0; i <= childNode.getParent().getSize(); i++)
            childNode.getParent().getTheChildAtIndex(i).setParent(childNode.getParent());
    }

     void setSplitParentNode(BTNode<K> node) {
    ...
    public void insert(K key) {
        ...
        if (!currentNode.isFull()) {
            ...
            BTNode<K> originalParentNode = getRestOfHalfKeys(currentNode);
            currentNode.addChild(0, newChildNode);
            currentNode.addChild(1, originalParentNode);
            originalParentNode.setParent(currentNode);
     ...
     int findChild(BTNode<K> node) {
        if (!node.equals(root)) {
            BTNode<K> ParentNode = node.getParent();

            for (int i = 0; i <= ParentNode.getSize(); i++) {
                if (ParentNode.getTheChildAtIndex(i).equals(node))
                    return i;
            }
        }
        return -1;
     ...
     BTNode<K> balanceDeletedNode(BTNode<K> node) {
        ...
        if (nodeIndex == 0) {
            currentNode = ParentNode.getTheChildAtIndex(1);
            flag = true;
        } else {
            currentNode = ParentNode.getTheChildAtIndex(nodeIndex - 1);
            flag = false;
        }
        ...
        if (currentSize > hNumber) {
            if (flag) {
                ...
                pair = ParentNode.getKey(nodeIndex - 1);
                node.addKey(0, pair);
                ParentNode.removeKey(nodeIndex - 1);
                ...
                ParentNode.addKey(nodeIndex - 1, pair);
                if (node.trueIfLastInternalNode()) {
                    node.removeChild(0);
                }
                System.out.println("Case 2a:");
            ...
            if(flag) {
                currentNode.addKey(0, ParentNode.getKey(0));
                ParentNode.removeKey(0);
...
//				stepMess.add("Merging");
                ...
                currentNode.addKey(currentNode.getSize(), ParentNode.getKey(nodeIndex - 1));
                ParentNode.removeKey(nodeIndex - 1);
                ParentNode.removeChild(nodeIndex);
                if (root.getSize() == 0) {
                    root = currentNode;
                    currentNode.setParent(nullBTNode);
                }
